...
import 'package:unity_test/View/Student/Home/HomePageStudent.dart';
...
class NavegationPageStudentController with ChangeNotifier {
  int selectedIndex = 2; // تعيين "الرئيسية" كالشاشة الافتراضية عند بدء التطبيق

  final List<Widget> studentScreens = [
    const SettingsScreen(userType: 'student'), // الإعدادات
    ...
    ChangeNotifierProvider(
        create: (context) =>
            HomeControllerStudent()..GetProfileStudent(context),
        builder: (context, child) => HomePageStudent()), // الرئيسية
    ChangeNotifierProvider(
