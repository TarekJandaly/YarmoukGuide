...
import 'package:unity_test/View/Employee/Home/HomePageEmployee.dart';
...
class NavegationPageEmployeeController with ChangeNotifier {
  int selectedIndex = 2; // تعيين "الرئيسية" كالشاشة الافتراضية عند بدء التطبيق

  final List<Widget> employeeScreen = [
    const SettingsScreen(userType: 'employee'), // الإعدادات
    ...
    ChangeNotifierProvider(
      create: (context) =>
          HomeControllerEmployee()..GetProfileEmployee(context),
      builder: (context, child) => HomePageEmployee(),
    ), // الرئيسية
    ChangeNotifierProvider(
