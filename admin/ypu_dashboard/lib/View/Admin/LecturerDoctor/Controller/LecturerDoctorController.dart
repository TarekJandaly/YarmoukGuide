...
class LecturerDoctorController with ChangeNotifier {
  List<Course> course = [];
  ...
  Future<void> GetDoctorCourses(BuildContext context, int id) async {
    schedulecourses = [];
    ...
    try {
      final response = await client.request(
        requestType: RequestType.GET,
        path: AppApi.GetDoctorCourses(id),
      );
      log(response.body);
  ...
  Future<void> AssignCoursesToDoctor(BuildContext context, int id) async {
    EasyLoading.show();
    try {
      final response = await client.request(
          requestType: RequestType.POST,
          path: AppApi.AssignCoursesToDoctor(id),
          body: jsonEncode(
  ...
  DialogRegisterCourse(BuildContext context, int id) {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ElevatedButton(
              onPressed: () => AssignCoursesToDoctor(context, id),
              child: Text("Next")),
          TextButton(
